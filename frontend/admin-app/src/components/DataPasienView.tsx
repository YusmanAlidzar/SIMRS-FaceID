...
export default function DataPasienView({
  ...
  const [formStatus, setFormStatus] = useState<Patient['status']>('Outpatient');
  const [formError, setFormError] = useState('');

  // Handle addition of a new patient record
  const handleSubmitNewPatient = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formName.trim() || !formAge.trim() || !formAddress.trim()) {
      setFormError('Harap lengkapi Nama, Usia, dan Alamat Anda.');
      return;
    }
    setFormError('');
    ...
    const newPatientObj: Patient = {
      ...
      name: formName,
      gender: formGender,
      age: formAge.endsWith('th') || formAge.includes('Thn') ? formAge : `${formAge}th`,
      address: formAddress,
    ...
    setFormPhone('');
    setFormAddress('');
    setFormStatus('Outpatient');
    onCloseAddPatientModal();
  };

  // Compile CSV for patients
  const handleExportCSV = () => {
    const header = 'ID Pasien,Nama Pasien,Jenis Kelamin,Usia,Alamat,Nomor Hp,Status,Tanggal Registrasi\r\n';
    const rows = patients
      .map(
        (p) =>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-8 glass-card rounded-2xl p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="space-y-1">
            <h3 className="text-xl font-bold text-primary">Patient Registry Overview</h3>
            <p className="text-xs text-on-surface-variant">
              Akses dan kelola basis data pasien terpusat untuk seluruh unit dan poliklinik RSUD.
            </p>
          </div>
          
          <div className="flex gap-4">
            <div className="text-center px-4 border-r border-outline-variant">
              <p className="text-2xl font-black text-primary">{patients.length + 1280}</p>
              <p className="text-[9px] font-bold text-outline uppercase tracking-wider">TOTAL ACTIVE</p>
            </div>
            <div className="text-center px-4">
              <p className="text-2xl font-black text-secondary">
                {patients.filter((p) => p.registeredDate === '2023-10-24').length + 42}
              </p>
              <p className="text-[9px] font-bold text-outline uppercase tracking-wider">NEW TODAY</p>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-2.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-4 py-1.5 w-48 border border-outline rounded-xl text-xs bg-white focus:outline-none focus:border-primary"
              />
            </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-primary text-white">
                <th className="px-5 py-3 text-xs font-bold uppercase tracking-wider">Patient ID</th>
                <th className="px-5 py-3 text-xs font-bold uppercase tracking-wider">Full Name</th>
                <th className="px-5 py-3 text-xs font-bold uppercase tracking-wider">Gender</th>
                <th className="px-5 py-3 text-xs font-bold uppercase tracking-wider">Age</th>
                <th className="px-5 py-3 text-xs font-bold uppercase tracking-wider">Address</th>
            ...
            <tbody className="divide-y divide-outline-variant/30">
              {paginatedPatients.length === 0 ? (
                ...
                paginatedPatients.map((p) => {
                  ...
                  let statusBadgeClass = 'bg-secondary-fixed text-on-secondary-fixed';
                  if (p.status === 'Inpatient') {
                    ...
                    statusBadgeClass = 'bg-error-container text-on-error-container font-black';
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-secondary-container/15 transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-xs text-outline truncate max-w-sm" title={p.address}>
                        {p.address}
                      </td>
                      <td className="px-5 py-3.5">
                        <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider inline-block ${statusBadgeClass}`}>
                          {p.status}
                        </span>
                      </td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="glass-card rounded-2xl p-5 flex items-center gap-4 hover:shadow-sm transition-shadow">
          ...
          <div>
            <h4 className="text-sm font-bold text-on-surface">Recent Changes</h4>
        ...
        <div className="glass-card rounded-2xl p-5 flex items-center gap-4 hover:shadow-sm transition-shadow">
          ...
          <div>
            ...
            <p className="text-xs text-outline mt-0.5">HIPAA Compliant System</p>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-primary/30 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl border border-outline-variant overflow-hidden animate-card-scale">
            ...
            <div className="bg-primary p-5 text-white flex justify-between items-center">
              <span className="text-sm uppercase tracking-wider font-bold">Registrasi Pasien Baru</span>
              <button
            ...
            <form onSubmit={handleSubmitNewPatient} className="p-5 space-y-4 max-h-[80vh] overflow-y-auto">
              {formError && (
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider px-1">Nama Lengkap</label>
                <input
                  type="text"
                  required
                  value={formName}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider px-1">Jenis Kelamin</label>
                  <select
                    value={formGender}
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider px-1">Usia / Umur</label>
                  <input
                    type="text"
                    required
                    value={formAge}
              ...
              {/* Phone text fields */}
              <div className="flex flex-col gap-1">
                <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider px-1">Nomor HP / Whatsapp</label>
                <input
                  type="text"
                  value={formPhone}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider px-1">Metode Perawatan</label>
                <select
                  value={formStatus}
                ...
                >
                  <option value="Outpatient">Outpatient (Rawat Jalan)</option>
                  <option value="Inpatient">Inpatient (Rawat Inap)</option>
                  <option value="Critical">Critical / UGD (Gawat Darurat)</option>
                </select>
              </div>

              {/* Address Area */}
              <div className="flex flex-col gap-1">
                <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider px-1">Alamat Tinggal Lengka</label>
                <textarea
                  rows={2}
                  required
                  value={formAddress}
